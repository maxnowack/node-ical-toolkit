...
ICSFileBuilder.prototype.toString = function () {
  ...
  for (var idx = 0; idx < this.events.length; idx++) {
    ...
    if (_event.allDay) {
      lines.push('DTSTART;VALUE=DATE' + _formatDate(moment(_event.start).startOf('day').toDate(), false, tzMode));
      lines.push('DTEND;VALUE=DATE' + _formatDate(moment(_event.end).endOf('day').toDate(), false, tzMode));
    } else {
